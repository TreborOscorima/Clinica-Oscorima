...
## 10. Estado al cierre (2026-08-15)

**HECHAS:** E0, E1, E2, E3, E5, E6, E7, E10 (GLB reales para dientes/rostro/cuerpo), **E11**.
Rama `feat/anatomia-cuerpo-3d` → **PR #8** (E7 + E11), CI en verde. Nada desplegado a producción (deploy AWS en espera hasta terminar de probar todo en local).

**PENDIENTES del motor:**

| # | Etapa | Qué falta | Esfuerzo |
|---|---|---|---|
| **E4** | Superficies del diente en 3D | Selección de **cara** (oclusal/vestibular/mesial/distal/lingual) sobre la pieza 3D → escribir en `caras` JSON (ya soportado por el modelo) + leyenda/colores por cara. | Medio |
| **E8** | Fotos + antes/después por zona | Asociar `Adjunto`/`SesionEstetica` a `zona_codigo`; comparativa antes/después por zona (reusa la galería C1 ya existente). | Bajo |
