...
| 2026-08-16 | **E9 HECHO (reporte de estética en PDF)** — Export imprimible del mapa estético por paciente, patrón `pdf_*` existente (espeja `pdf_odontograma`). `services/anatomia.py`: helper `severidad_label` (0–4 → legible). `services/estetica_mapa.py`: `datos_export(session, clinica_id, paciente_id)` reúne paciente + evaluaciones (con `severidad_label`) + procedimientos con sus puntos (resolviendo `producto_nombre` vía `_nombres_productos`) + fotos agrupadas por zona_label × momento (antes/durante/después) + totales; `NotFoundError` si el paciente no existe. `services/pdf_estetica.py` **nuevo**: `generar_estetica_pdf(**datos) → bytes` (ReportLab, A4 portrait; secciones: datos del paciente, resumen en píldoras, tabla de evaluaciones, procedimientos c/tabla de puntos producto/lote/cantidad/coord, y fotos antes/después por zona; tolera datos vacíos). `clinica_app.py`: ruta **`/api/estetica/pdf`** (token efímero + chequeo de clínica, igual que odontograma → `Response` inline `application/pdf`). `pages/mapa_estetico.py`: botón **"Exportar PDF"** en el `page_header` (sólo con paciente cargado). **4 tests nuevos** (estructura del export + nombre de producto + severidad_label + fotos por zona; paciente inexistente; PDF `%PDF-` con y sin datos); **suite 302 verde**, ruff + compileall limpios. **Verificado IN-CONTAINER** (imagen rebuildeada + `life_web` purgado + GLBs recopiados): backend baked (route + `datos_export`/`generar_estetica_pdf`/`severidad_label`, reportlab 5.0.0), y **E2E HTTP real** — con datos creados para el paciente 13 y token minteado, `GET /api/estetica/pdf` → **HTTP 200**, `Content-Type application/pdf`, `Content-Disposition inline; filename="mapa-estetico-paciente13.pdf"`, 3235 bytes `%PDF-`; BD dejada limpia (datos soft-deleted). Link presente en el bundle `_mapa-estetico` compilado. | `feat(anatomia): E9 reporte de estética en PDF (ReportLab)` |
| 2026-08-16 | **E4 HECHO (detalle por cara del diente) + baseline E2E de integración** — Editor de caras en el modal del odontograma (compartido por 2D y 3D vía `on_pick_3d`), escribiendo el JSON `caras` que el modelo `PiezaDental` ya soportaba. `state/odontograma.py`: vars `sel_caras`/`cara_pincel`, computed `caras_view` (las 5 caras con su color/estado), handlers `set_cara_pincel`/`pintar_cara` (toggle: pinta con el pincel; re-toca = limpia) / `limpiar_caras`; `abrir_pieza` carga las caras, `guardar_pieza` las persiste. `pages/odontograma.py`: **cruz odontológica 3×3** clicable (V arriba; M·O·D; P abajo) + selector de **pincel** (estado a pintar) + "Limpiar caras". `services/pdf_odontograma.py`: helper puro `_caras_texto` + el filtro de hallazgos ahora **incluye piezas sanas con detalle por cara** (antes solo `estado != sano`) → el detalle por cara sale en el PDF. **4 tests nuevos** (`_caras_texto` formatea/vacío, PDF incluye pieza sana-con-caras, `datos_export` propaga caras) → **suite 306 verde**, ruff + compileall limpios. **Verificado IN-APP real** (rebuild + purga `life_web` + 5 GLB recopiados; login `admin@demo.com`, `/odontograma?paciente_id=13`): abro pieza 16 → modal con "Detalle por cara" + 5 botones V/M/O/D/P; pinto **Oclusal=caries** (regla emotion `#ef4444` + texto blanco) y, cambiando el pincel, **Mesial=obturado** (`#3b82f6`); Guardar → BD `piezas_dentales` persiste `caras={'oclusal':'caries','mesial':'obturado'}` con `estado=sano` (el detalle por cara es independiente del estado global); PDF odontograma HTTP 200 con esa pieza; cleanup a sano. **GOTCHA confirmado:** con el pane WebGL sin componer, `getComputedStyle`/screenshots devuelven valores stale → verificar por **reglas CSS emotion** (`document.styleSheets`), no por computed style. **Motor anatómico COMPLETO.** | `feat(anatomia): E4 detalle por cara del diente + baseline E2E de integración` |
...
**HECHAS:** E0, E1, E2, E3, **E4**, E5, E6, E7, **E8**, **E9**, E10 (GLB reales para dientes/rostro/cuerpo), **E11**.
**✅ MOTOR ANATÓMICO COMPLETO** — todas las etapas E0–E11 hechas y verificadas.
...
**E2E de integración (baseline reproducible):** `scripts/e2e_integracion.py` — corre DENTRO del contenedor (`docker exec -e PYTHONPATH=/app tuwayki_life python /tmp/e2e_integracion.py` tras `docker cp`) y atraviesa los flujos reales integrados (auth → odontograma+caras+PDF → mapa estético eval/proc/punto/foto+PDF → serving de adjuntos → seguridad de endpoints 401/403 → aislamiento multi-tenant) contra la app viva + BD real + HTTP, limpiando todo lo que crea. **22/22 PASS** antes y después de E4 (cero regresiones). Correrlo tras cada etapa que toque backend/rutas.
