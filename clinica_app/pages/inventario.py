    ...
    return rx.cond(
        ...
        rx.el.div(
            ...
            rx.el.div(
                ...
                rx.el.div(
                    _campo("Nombre *", "text", InventarioState.form_nombre, InventarioState.set_var("form_nombre")),
                    _campo("SKU", "text", InventarioState.form_sku, InventarioState.set_var("form_sku")),
                    rx.el.div(
                        _campo("Precio costo", "number", InventarioState.form_precio_costo,
                               InventarioState.set_var("form_precio_costo"), "0.00"),
                        _campo("Precio venta", "number", InventarioState.form_precio_venta,
                               InventarioState.set_var("form_precio_venta"), "0.00"),
                        class_name="grid grid-cols-2 gap-3",
                    ),
                    rx.el.div(
                        _campo("Stock actual", "number", InventarioState.form_stock_actual,
                               InventarioState.set_var("form_stock_actual"), "0"),
                        _campo("Stock mínimo", "number", InventarioState.form_stock_minimo,
                               InventarioState.set_var("form_stock_minimo"), "0"),
                        class_name="grid grid-cols-2 gap-3",
                    ),
            ...
            rx.el.div(
                ...
                rx.el.div(
                    ...
                    rx.el.select(
                        ...
                        on_change=InventarioState.set_var("form_mov_tipo"),
                        class_name="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500",
                    ),
                    class_name="mb-4",
                ),
                _campo("Cantidad *", "number", InventarioState.form_mov_cantidad,
                       InventarioState.set_var("form_mov_cantidad"), "0"),
                rx.el.div(class_name="mb-4"),
                _campo("Motivo", "text", InventarioState.form_mov_motivo,
                       InventarioState.set_var("form_mov_motivo"), "Opcional..."),
                rx.el.div(
...
def _fila_producto(p: dict) -> rx.Component:
    return rx.el.tr(
        rx.el.td(p["sku"] or "—", class_name="px-4 py-3 text-xs text-gray-500 font-mono"),
        ...
        rx.el.div(
            rx.el.div(
                rx.el.table(
                    ...
                    rx.el.tbody(rx.foreach(InventarioState.productos, _fila_producto)),
